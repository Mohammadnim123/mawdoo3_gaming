...
Tweaks may carry a reference image (screenshot/mockup); it is validated and
normalized here (Pillow: sniff, bomb guard, 1568px long-edge cap, WebP
re-encode) BEFORE a job exists, so a bad image is a clean 422 with no job row.
"""
...
class StartTweakUseCase:
    ...
    async def execute(
        self, game_id: str, instruction: str, image_base64: str | None = None
    ) -> GenerationJob:
        ...
        # Validate + normalize the optional reference image before creating
        # the job — a bad attachment is a 422, never a failed job. CPU-bound
        # Pillow work runs off the event loop.
        image_b64: str | None = None
        if image_base64:
            image_b64 = await asyncio.to_thread(normalize_image_b64, image_base64)
        ...
        self._runner.submit(
            self._run_generation.execute(job, image_b64=image_b64), name=f"tweak:{job.id}"
        )
        return job
